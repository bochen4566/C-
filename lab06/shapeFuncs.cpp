...
double distanceBetween(Point p, Point q) {
  double res;
  res = sqrt(pow((p.x-q.x), 2.0) + pow((p.y-q.y), 2.0));
  return res; // @@@ STUB
...
void initPoint(struct Point *p, double xVal, double yVal) {
  p->x = xVal;
  p->y = yVal;//return; //@@@ for a void function, the stub is just a bare return that does nothing
}
...
string boxToString(Box b, int precision) {
  ...
  return oss.str();// SAMPLE FORMAT: "ul=(3.4,-5),w=5,h=7"
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
  ...
  return (pointsApproxEqual(b1.ul, b2.ul)&&ApproxVal(b1.height, b2.height)&&ApproxVal(b1.width, b2.width));
...
void initBox(struct Box *b, double ulx, double uly, double w, double h)
{
  ...
  b->ul.y = uly;
  b->height = h;
  b->width = w; // @@@ For a void function a "naked return" is a "do nothing" stub
}
